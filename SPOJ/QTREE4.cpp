...
struct centroid_decomposition {
    int N;
    vvi centroid_graph;
    vi node_mapping, reverse_mapping, subtree_size, parent;
    ...
    void decompose(int loc, const int par, const vvi& graph) {
        ...
        if( par != -1) {
            ...
            parent[ node_mapping[C] ] = node_mapping[par];
        }
...
inline int calculate_distance(int u, int v, const lca& memo) {
...
int main()
{
    ...
    for(int i = 0; i < n; ++i) {
        ...
        while(cur != -1) {
            ...
            ancDist[i][lvl] = calculate_distance(old_node_value, old_cur_value, arv);
            cur = father[cur];
            ++lvl;
        }
    }
