...
int main()
{
    ...
    cin >> n;
    vector<pii> o(n), c(n);
    rep(i,0,n) cin >> o[i].first >> o[i].second;
    rep(i,0,n) cin >> c[i].first >> c[i].second;
    map< pii, int > countOrig;
    map< pii, int > cpy;
    rep(i,0,n) countOrig[c[i]]++;
    rep( fstC, 0, n) {
        cpy = countOrig;
        pii T = make_pair( o[0].first + c[fstC].first, o[0].second + c[fstC].second );
        bool ok = true;
        rep(i, 0, n) {
            pii diff = { T.first - o[i].first, T.second - o[i].second };
            if( cpy[diff] > 0) cpy[diff]--;
            else {
                ok = false;
                break;
            } 
